...
import CollegeManager from './components/CollegeManager.jsx';
...
function App() {
  ...
  return (
    <div className="dynamic-shell relative min-h-screen text-slate-900 overflow-x-hidden">
      <header className="glass-header sticky top-0 z-40">
        <div className="mx-auto w-full max-w-[95vw] px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <img src="/assets/images/SUD-01.png" alt="Oliy sud logotipi" className="h-16 w-16 object-contain drop-shadow-sm transition duration-300 hover:scale-105" />
              <div className="min-w-0">
                <Link to="/" className="block text-xl font-semibold tracking-tight text-slate-900 transition hover:text-indigo-700 sm:text-2xl">Oliy sud</Link>
              ...
              <div className="hidden md:flex items-center gap-4 ml-6 text-sm text-indigo-700">
                <Link to="/" className="rounded-full px-3 py-2 font-medium transition hover:bg-indigo-100 hover:text-slate-900">Jamoat</Link>
                {token ? <Link to="/admin" className="rounded-full px-3 py-2 font-medium transition hover:bg-indigo-100 hover:text-slate-900">Dashboard</Link> : <Link to="/login" className="rounded-full px-3 py-2 font-medium transition hover:bg-indigo-100 hover:text-slate-900">Kirish</Link>}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-md bg-white p-2 text-indigo-700 shadow-sm transition hover:bg-indigo-50 hover:scale-105 md:hidden"
                aria-label="Menyu"
              >
              ...
              {token && user && (
                <div className="relative flex-shrink-0 flex items-center gap-3" ref={profileRef}>
                  ...
                  <button
                    ...
                    className="flex h-12 w-12 items-center justify-center rounded-full bg-indigo-100 text-lg font-semibold text-slate-900 shadow-sm transition hover:scale-105 hover:bg-indigo-200"
                  ...
                  {profileMenuOpen && (
                    <div className="animate-page absolute top-full right-0 mt-2 z-50 w-52 rounded-2xl border border-indigo-100 bg-white p-2 shadow-lg shadow-indigo-100/70">
  ...
  >
    Tizimdan chiqish
          ...
          {/* Mobile nav panel */}
          <div className={`${mobileMenuOpen ? 'block' : 'hidden'} animate-page mt-3 md:hidden`}>
            <nav className="space-y-2 rounded-2xl border border-indigo-100 bg-white p-4 text-sm text-indigo-700 shadow-sm">
      ...
      <main className="animate-page relative mx-auto w-full max-w-[95vw] p-4 sm:p-6 lg:p-8">
        <Routes>
          ...
          <Route path="/admin" element={<AdminLayout user={user} />}>
            ...
            <Route path="colleges" element={<CollegeManager user={user} />} />
