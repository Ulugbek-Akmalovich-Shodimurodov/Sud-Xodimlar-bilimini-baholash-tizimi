...
router.post('/', authenticateToken, permitPermission(PERMISSIONS.CRITERIA_MANAGE), async (req, res, next) => {
  try {
...
router.put('/:id', authenticateToken, permitPermission(PERMISSIONS.CRITERIA_MANAGE), async (req, res, next) => {
  try {
    const { error, value } = criteriaSchema.validate(req.body);
    if (error) return res.status(400).json({ error: error.message });
...
router.delete('/:id', authenticateToken, permitPermission(PERMISSIONS.CRITERIA_MANAGE), async (req, res, next) => {
  try {
    await ensureCriteriaTable();
